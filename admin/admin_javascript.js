...
// ==================== FUNCIÓN PARA LIMPIAR CAMPOS TEMPORALES QR ====================
...
window.resetearBloqueoStock = function() {
    ...
    if (window.ingredienteEditandoId && stockInput && !stockInput.disabled) {
        console.log('Restaurando bloqueo...');
...
// ==================== TASA DE HOY — PRIMER INICIO DEL DÍA ====================
...
window.toggleDisponiblePlatillo = async function(id, disponible) {
    try {
        ...
        if (disponible) {
            ...
            window.mostrarToast(`⛔ Platillo "${item?.nombre}" ahora está NO DISPONIBLE (se mostrará como AGOTADO en el menú del cliente)`, 'warning');
        }
    } catch(e) {
...
// ==================== BACK-NAV + CONFIRMACIÓN DE SALIDA ====================
(function() {
    var _depth = 2;
    ...
    if (_btnNo) _btnNo.addEventListener('click', function() {
        _ocultarAviso(); _push();
    });
...
window._notificarAdminStockCritico = async function(ingredienteNombre) {
    try {
        ...
        window.mostrarToast(`⚠️ Alerta enviada: stock crítico en ${ingredienteNombre}`, 'warning');
...
// ==================== ACTUALIZAR STOCK CRÍTICO HEADER ====================
...
window.categoriasMenu = SUBCATEGORIAS;
