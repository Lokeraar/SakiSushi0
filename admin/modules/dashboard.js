...
export function dashboardComponent() {
    return {
        ...
        async guardarConfiguracion() {
            this.loading = true;
            try {
                await supabaseClient.client.from('config').update({
        ...
        async actualizarVentasHoy() {
            ...
            const { data: ventas, error } = await supabaseClient.client
                .from('ventas')
                .select('pedido_id')
            ...
            const { data: pedidos, error: pedErr } = await supabaseClient.client
                .from('pedidos')
                .select('*')
                .in('id', pedidoIds);
            if (pedErr) return;
