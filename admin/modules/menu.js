...
export function menuComponent() {
    return {
        ...
        async savePlatillo() {
            ...
            try {
                if (this.editMode) {
                    await supabaseClient.client.from('menu').update(data).eq('id', this.form.id);
                    showToast('Platillo actualizado', 'success');
                } else {
                    await supabaseClient.client.from('menu').insert([data]);
        ...
        async deletePlatillo(id) {
            ...
            try {
                await supabaseClient.client.from('menu').delete().eq('id', id);
        ...
        async toggleDisponible(id, disponible) {
            try {
                await supabaseClient.client.from('menu').update({ disponible }).eq('id', id);
