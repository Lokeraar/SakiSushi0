...
export function usuariosComponent() {
    return {
        ...
        async guardarUsuario() {
            ...
            try {
                if (this.editMode) {
                    ...
                    const { data: hashed, error: hashErr } = await supabaseClient.client
        ...
        async toggleActivo(usuario) {
            try {
                await supabaseClient.client.from('usuarios').update({ activo: !usuario.activo }).eq('id', usuario.id);
        ...
        async eliminarUsuario(id) {
            ...
            try {
                await supabaseClient.client.from('usuarios').delete().eq('id', id);
