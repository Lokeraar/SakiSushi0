// admin-reportes.js - Reportes y gráficos (corregido)
(function() {
window.cargarReportes = async function() {
    ...
    try {
        ...
        if (hasta) { 
            ...
            query = query.lt('fecha', h.toISOString()); 
        }
        ...
        window.mostrarToast('Error cargando reportes', 'error'); 
    }
};
...
window.actualizarEstadisticasReportes = function(pedidos) {
    ...
    pedidos.forEach(p => { 
        if (p.items) p.items.forEach(item => { 
            platillosCount[item.nombre] = (platillosCount[item.nombre] || 0) + (item.cantidad || 0); 
        }); 
    });
    let platilloTop = '-', maxCount = 0;
    for (const [n, c] of Object.entries(platillosCount)) { 
        if (c > maxCount) { maxCount = c; platilloTop = n; } 
    }
    
    const tasa = window.configGlobal?.tasa_efectiva || 400;
    
    // Función auxiliar segura para actualizar elementos
    const _s = (id, v) => { 
    ...
    // Nueva tarjeta: Cantidad de Pedidos
...
window.actualizarGraficos = function(pedidos) {
    ...
    pedidos.forEach(p => { 
        const f = new Date(p.fecha).toISOString().split('T')[0]; 
        if (ventasPorDia.hasOwnProperty(f)) ventasPorDia[f] += p.total || 0; 
    });
    ...
    const metodos = {};
    pedidos.forEach(p => {
        if (p.pagos_mixtos) p.pagos_mixtos.forEach(pago => { 
