...
(function() {
    ...
    window.renderizarMesoneros = async function() {
        ...
        let pendientes = {};
        try {
            // Obtener DEUDA PENDIENTE (propinas con entregado = false)
            const { data: allProp } = await window.supabaseClient
                .from('propinas').select('mesonero_id, monto_bs').eq('entregado', false);
            (allProp || []).forEach(p => {
                pendientes[p.mesonero_id] = (pendientes[p.mesonero_id] || 0) + (p.monto_bs || 0);
            });
