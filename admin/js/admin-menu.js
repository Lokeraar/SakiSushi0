...
window.renderizarMenu = function(filtro) {
    ...
    if (!items.length) {
        grid.innerHTML = `<p style="color:var(--text-muted);text-align:center;padding:2rem;">${filtro ? 'Sin resultados para "' + filtro + '"' : 'No hay platillos registrados.'}</p>`;
        return;
    }
    ...
    items.forEach(item => {
        ...
        card.innerHTML = `
            ...
            <div class="mc2-stock-line">
                ...
                <span class="mc2-badge ${disponibleFinal ? 'mc2-badge-ok' : 'mc2-badge-off'}">${disponibleFinal ? 'Disponible' : 'No disponible'}</span>
            </div>
            <div class="mc2-tags">
        ...
        card.querySelectorAll('.ing-tag[data-ingrediente-id]').forEach(tag => {
            tag.addEventListener('click', (e) => { 
                e.stopPropagation(); 
                window._irAIngrediente(tag.dataset.ingredienteId); 
            });
        });
...
window.editarPlatillo = function(id) {
    ...
    if (_lblD) { 
        _lblD.textContent = platillo.disponible ? 'Sí' : 'No'; 
        _lblD.style.color = platillo.disponible ? 'var(--success)' : 'var(--text-muted)'; 
    }
    ...
    for (const entry of _otrosNuevos) {
        ...
        try {
            ...
            if (!errIng) {
                ...
                console.log('✅ Created new ingredient:', entry.nombreOtro);
            } else { 
